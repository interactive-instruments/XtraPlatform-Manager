...
const mapStateToProps = (state /*, props*/) => {
  return {
    ...
    authExpired:
      state.entities.expired ||
      Object.values(state.queries).some((query) => query.status === 401),
  };
};
        ...
        {user ? (
          <Box direction="row" fill>
            <NavSidebar
              ...
              onClose={navToggle.bind(null, false)}
            />
            <Box flex fill="vertical">
              {user.forceChangePassword || children}
            </Box>
          </Box>
        ) : (
          <Box direction="row" fill>
