...
import {
  LinkPrevious as LinkPreviousIcon,
  More as MoreIcon,
  Globe,
} from "grommet-icons";
...
@connectRequest((props) =>
  ServiceApi.getServiceQuery(props.urlParams.id, {
    forceReload: true,
    secured: props.appConfig.secured,
  })
)
@connect(
  (state, props) => {
  ...
  (dispatch, props) => {
    return {
      ...
      updateService: (serviceId, update) => {
        ...
        dispatch(
          mutateAsync(
            ServiceApi.updateServiceQuery(serviceId, update, {
              secured: props.appConfig.secured,
            })
          )
        ).then((result) => {
      ...
      deleteService: (service) => {
        dispatch(
          mutateAsync(
            ServiceApi.deleteServiceQuery(service, {
              secured: props.appConfig.secured,
            })
          )
        ).then((result) => {
...
export default class Service extends Component {
  ...
  render() {
    ...
    const selectedTab =
      ...
      urlQuery.tab &&
      Math.max(
        Object.keys(editTabs).findIndex((label) => label === urlQuery.tab),
        0
      );
    ...
    const mappingStatus =
      ...
      service.featureProvider.mappingStatus;
    if (
      mappingStatus &&
    ...
    return service /*<Animate
      ...
      <Box fill={true}>
        ...
        <Tabs
          ...
          onActive={(index) => this._onTabSelect(Object.keys(editTabs)[index])}
        >
          {editTabs &&
